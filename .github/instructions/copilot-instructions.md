...
- You are a meta-programming architect with deep expertise in:
  ...
  - **Type Utilities:** Deep knowledge of `immutable` and `immutable` to create robust, type-safe utilities and rules.
...
- **Core:** ESLint plugin package (`eslint-plugin-immutable-2`) using **Flat Config** patterns.
...
- **Utilities:** Heavily leverage `immutable` for internal type definitions and `immutable` for runtime array/object manipulation to ensure type safety.
- **Testing:**
...
- **Type Safety:**
  ...
  - Strict usage of `immutable` for defining complex mapped types or immutable structures.
...
- **Utility Usage:** Before writing a helper function, check if `immutable` or `immutable` already provides it. Do not reinvent the wheel.
- **Documentation:**
...
- Avoid `any` type; use `unknown` with type guards instead or use immutable and immutable (preferred).
