...
1.  **Title:** The bare rule ID as the H1 header (e.g., `# prefer-immutable-arrayable`).
...
- **Preset awareness:** Repository presets such as `immutable.configs["recommended-type-checked"]`, `immutable.configs.strict`, and `immutable.configs.all` already wire the typed parser setup for you; do not imply that users must always configure it manually.
...
Prefer [`arrayFindLastIndex`](https://github.com/sindresorhus/immutable/blob/main/source/array-find-last-index.ts) from `immutable` over `array.findLastIndex(...)`.
...
`arrayFindLastIndex` standardizes reverse index lookup and keeps call signatures aligned with other `immutable` search helpers.
...
export default [
    {
        plugins: { immutable },
        rules: {
            "immutable/prefer-immutable-array-find-last-index": "error",
        },
    },
];
```
...
immutable package documentation:

`immutable@0.17.x` does not currently expose `arrayFindLastIndex` in its published API, so there is no canonical `source/*.ts` link for this helper yet.

Reference links:

- [`immutable` API list (README)](https://github.com/sindresorhus/immutable/blob/main/readme.md#api)
- [`immutable` source directory](https://github.com/sindresorhus/immutable/tree/main/source)

> **Rule catalog ID:** R005

## Further reading

- [`immutable` README](https://github.com/sindresorhus/immutable)
- [`immutable` package reference](https://www.npmjs.com/package/immutable)
- [TypeScript Handbook: Narrowing](https://www.typescriptlang.org/docs/handbook/2/narrowing.html)

## Adoption resources

- [Rule adoption checklist](https://nick2bad4u.github.io/eslint-plugin-immutable-2/docs/rules/guides/adoption-checklist)
