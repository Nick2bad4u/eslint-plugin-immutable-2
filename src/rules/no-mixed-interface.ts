...
const isFunctionPropertySignature = (
    ...
    return node.typeAnnotation.typeAnnotation.type === AST_NODE_TYPES.TSFunctionType;
};

/** `no-mixed-interface` rule implementation. */
const noMixedInterfaceRule: ReturnType<typeof createRule<readonly [], "generic">> =
    createRule<readonly [], "generic">({
        create(context) {
            return {
                TSInterfaceDeclaration(node) {
                    let previousMemberType: TSESTree.TypeElement["type"] | undefined;
